...
namespace App.Areas.Admin.Identity.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Profile(bool isEdit = false)
        {
            ...
            ViewData["UserRole"] = roles.FirstOrDefault() ?? "User";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateAvatar(string croppedImage)
        {
            if (string.IsNullOrEmpty(croppedImage))
            {
                TempData["Error"] = "Không có hình ảnh được chọn!";
                return RedirectToAction("Profile");
            }
            ...
            // Optional: Save to wwwroot/uploads/avatars
            var base64Data = Regex.Match(croppedImage, @"data:image/(?<type>.+?),(?<data>.+)").Groups["data"].Value;
            var imageBytes = Convert.FromBase64String(base64Data);

            var fileName = $"{user.Id}_avatar.png";
            var filePath = Path.Combine(env.WebRootPath, "uploads", "avatars", fileName);
